...
function main() {
  ...
  var MSG = {
        ...
        enable:       '启用',
        ex:           '导出',
        exDlg:        '选择一个文件夹来导出 TXT...',
        exHint:       '将活动选项卡中的名称导出到 TXT 文件中',
        exSuccess:    '您的文件*已成功创建',
        find:         '查找',
        findHint:     '支持正则表达式',
        findTitle:    '查找和替换',
        im:           '导入',
        imDlg:        '选择 TXT 文件...',
        imHint:       '将名称从 TXT 文件导入到活动选项卡。每个名称另起一行',
        imSuccess:    '您的文件*已成功导入活动选项卡',
        nameAb:       '画板名称',
        nameLyr:      '图层名称',
        namePath:     '对象名称',
        ok:           'OK',
        ...
        preSuffAb: '占位符:\n' +
                      ...
                      PH.height + ' - 画板高度,\n' +
                      PH.units + ' - 长度单位,\n' +
                      PH.numUp.replace(/\d/g, absLength) + ' - 自动编号从,\n' +
                      PH.numDown.replace(/\d/g, absLength) + ' - 数量减少,\n' +
                      PH.color + ' - 文件颜色空间,\n' +
                      PH.date + ' - 当前日期为 YYYYMMDD,\n' +
                      PH.fName + ' - 文件名',
        preSuffLyr:   '占位符:\n' +
                      PH.numUp.replace(/\d/g, lyrsLength) + ' - 自增编号,\n' + // Ascending
                      PH.numDown.replace(/\d/g, lyrsLength) + ' - 自减编号,\n' + // Descending
                      PH.fName + ' - 文件名',
        preSuffPath:  '占位符:\n' +
                      PH.numUp.replace(/\d/g, selLength) + ' - 自增编号, ' + // Ascending
                      PH.numDown.replace(/\d/g, selLength) + ' - 自减编号, ' + // Descending
                      PH.width + ' - 对象宽度, ' +
                      PH.height + ' - 对象高度, ' +
                      PH.units + ' - 标尺单位',
      };
