...
function main() {
  ...
  var selToPrntRb = tgtPnl.add('radiobutton', undefined, '将选定对象添加到父画板');
      selToPrntRb.value = isSelExists;
      selToPrntRb.enabled = isSelExists;
  var selToActRb = tgtPnl.add('radiobutton', undefined, '将选定对象添加到活动画板');
      selToActRb.enabled = isSelExists;
  var toAllRb = tgtPnl.add('radiobutton', undefined, '将对象作为组添加到父画板');
      toAllRb.value = !isSelExists;

  // Reference point
  var refPnl = wrapper.add('panel', undefined, '对齐点');
  ...
  // Bounds
  var bndsPnl = win.add('panel', undefined, '对象尺寸');
      bndsPnl.orientation = 'row';
      bndsPnl.alignment = 'fill';
      bndsPnl.margins = [10, 15, 10, 10];

  var geoRb = bndsPnl.add('radiobutton', undefined, '几何边界');
      geoRb.helpTip = '对象的边界（不包括笔触宽度和效果）';
      geoRb.value = true;
  var visRb = bndsPnl.add('radiobutton', undefined, '可见边界（笔触和效果）');
      visRb.helpTip = '对象的可见范围，包括笔触宽度和效果';

  // Buttons
  var footer = win.add('group');
      footer.alignment = 'fill';

  var copyright = footer.add('statictext', undefined, '\u00A9 Sergey Osokin. 汉化: Hwangzhun | Visit Github'),
      cancel, ok;

  var notice = win.add('statictext', undefined,
    '此界面为社区汉化版，功能与原版一致\n问题反馈请至汉化仓库'
  );

  if (CFG.isMac) {
    cancel = footer.add('button', undefined, '取消', { name: 'cancel' });
  ...
  function okClick() {
    ...
    if (tmpSel.length || toAllRb.value) {
      ...
      try {
        ...
        alert(err + ': on line ' + err.line, '脚本错误', true);
      }
    ...
    switch (true) {
      case /app/g.test(arg):
        if (!/illustrator/i.test(app.name)) {
          alert('错误的应用程序，请从 Adob​​e Illustrator 运行脚本', '脚本错误');
          return false;
        }
        break;
      case /version/g.test(arg):
        ...
        if (parseFloat(app.version) < rqdVers) {
          alert('应用程序版本错误\n抱歉，脚本仅适用于 Illustrator v.' + rqdVers + ' 以及后来版本', '脚本错误');
          return false;
        }
        break;
